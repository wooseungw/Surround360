...
from transformers import Blip2PreTrainedModel, Blip2Config, Blip2VisionModel, Blip2QFormerModel, AutoModelForCausalLM
...
class SurroundBlip(Blip2PreTrainedModel, GenerationMixin):
    ...
    def _compute_overlap_loss(self, vision_outputs: BaseModelOutput, B: int, P: int) -> torch.Tensor:
        ...
        image_embeds = vision_outputs.last_hidden_state; S, D = image_embeds.shape[1], image_embeds.shape[2]
        num_patches = S - 1 if (S > 1 and (S-1)**0.5 == int((S-1)**0.5)) else S
        if num_patches <= 0: return torch.tensor(0.0, device=image_embeds.device)
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, **kwargs):
        ...
        # 첫 번째 스텝: 비전 처리를 포함하여 `inputs_embeds`를 만듦
